...
class MiniGraph::Core::GraphTest < Minitest::Test
  parallelize_me!

  def create_directed_graph(vertices)
    MiniGraph::Core::Graph.new(vertices, directed: true)
  end

  def create_undirected_graph(vertices)
    MiniGraph::Core::Graph.new(vertices)
  end
  ...
  test '#add_edge raises an error if origin index exceeds the bounds of the graph vertices' do
    ...
    assert_raises MiniGraph::Core::Error::InvalidIndexError do
      graph.add_edge(5, 0)
    end
  end
  ...
  test '#add_edge allows instances of MiniGraph::Core::Edge to be added' do
    graph = create_directed_graph [1,2,3,4,5]
    edge  = MiniGraph::Core::Edge::Directed.new(2,3)
    graph.add_edge(edge)
  ...
  test '#add_edge raises an error if an undirected edge is added to a directed graph' do
    ...
    edge  = MiniGraph::Core::Edge::Undirected.new(2,3)

    assert_raises MiniGraph::Core::Error::InvalidEdgeTypeError do
      graph.add_edge(edge)
    end
  end
